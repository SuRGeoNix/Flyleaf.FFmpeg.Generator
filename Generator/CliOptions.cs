...
{
    [Option('n', "namespace", Default = "Flyleaf.FFmpeg", HelpText = "The namespace that will contain the generated symbols.")]
    public string Namespace { get; set; }

    [Option('c', "class", Default = "Raw", HelpText = "The name of the class that contains the FFmpeg unmanaged method calls.")]
    public string ClassName { get; set; }

    [Option('i', "input", Required = false, HelpText = "The path to the directory that contains the FFmpeg header and binary files " + "(must have the default structure).")]
    public string FFmpegDir { get; set; }

    [Option('h', "headers", Required = false, HelpText = "The path to the directory that contains the FFmpeg header files.")]
    public string FFmpegIncludesDir { get; set; }

    [Option('b', "bin", Required = false, HelpText = "The path to the directory that contains the FFmpeg binaries.")]
    public string FFmpegBinDir { get; set; }

    [Option('o', "output", Required = false, HelpText = "The path to the directory where to output the generated files.")]
    ...
    //[Option("ffmpeg-binary-url", Required = false, Default = "https://github.com/BtbN/FFmpeg-Builds/releases/download/latest/ffmpeg-n8.1-latest-win64-gpl-shared-8.1.zip")]
    //[Option("ffmpeg-binary-url", Required = false, Default = "https://www.gyan.dev/ffmpeg/builds/ffmpeg-release-full-shared.7z")]
    [Option("ffmpeg-binary-url", Required = false, Default = "https://github.com/BtbN/FFmpeg-Builds/releases/download/latest/ffmpeg-n9.0-latest-win64-gpl-shared-9.0.zip")]
    ...
    public void Normalize()
    {
        ...
        if (string.IsNullOrWhiteSpace(OutputDir)) OutputDir = Path.Combine(solutionDir, "Bindings");
    ...
    private void SetupFFmpegBinaries(string solutionDir, string ffmpegBinaryUrl)
    {
        ...
        FFmpegDir = Path.Combine(solutionDir, ".cache");
        FFmpegIncludesDir = Path.Combine(FFmpegDir, "include");
        FFmpegBinDir = Path.Combine(FFmpegDir, "bin");
